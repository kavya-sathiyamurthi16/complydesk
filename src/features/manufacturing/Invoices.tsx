...
export function ManufacturingInvoices() {
  ...
  const fetchInvoices = async (showLoader: boolean) => {
    try {
      if (showLoader) setIsLoading(true)
      setError(null)
      const data = await getInvoices()
      console.log('Invoices fetched:', data.length, 'rows')
      setInvoices(data)
    } catch (err) {
  ...
  useEffect(() => {
    ...
    // Refetch whenever the window regains focus (user comes back to tab)
                        ...
                        >
                          ...
                          <td className="py-4 px-6 text-sm text-gray-700">
                            <div className="font-medium text-gray-900">
                              {invoice.sellerLegalName || invoice.vendorName || 'Unknown'}
                            </div>
                            <div className="text-xs text-gray-500">
                              {invoice.sellerGstin || 'Seller GSTIN not provided'}
                            </div>
                          </td>
                          <td className="py-4 px-6 text-sm font-mono text-gray-700">
                            {invoice.buyerGstin || 'Buyer GSTIN not provided'}
                          </td>
                          <td className="py-4 px-6 text-sm font-mono text-right text-gray-900">
                            ₹{invoice.amount.toLocaleString()}
                          </td>
                          <td className="py-4 px-6 text-sm text-gray-700 font-mono">
                            {invoice.ewayBillNo || invoice.eway_status || 'E-way not provided'}
                          </td>
